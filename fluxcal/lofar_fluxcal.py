...
#              where only single station was used (e.g. FE). However, for now
#              Jones matrix values for the reference CasA observation are
#              still calculated for the CS001
# 05.08.2016 - Vlad Kondratiev
#              casa_beamcorr is now a dictionary with CasA correction factors
#              for all the CS and INTL stations. For RS stations mscorpol is
#              currently failing. So, now user can choose the reference station
#              with --station option even if more than 1 stations were used.
#              Default is now made CS002. If only one station was used, this station
#              will be default one unless it is specified with --station
